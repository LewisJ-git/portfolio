			...
			<MotionElement
				animation={
					new Animate({
						...
						transition: new Transitions(0.7, bezierSharp, 1.6).transition
					}).variant
				}
			...
			<MotionElement
				animation={
					new Animate({
						initialX: 200,
						opacity: new Fading(1, "fast").opacity,
						transition: new Transitions(0.7, bezierSharp, 1.3).transition
					}).variant
				}
				classname="text-midground absolute z-10 pt-[10px] pl-[30px] font-barcode top-[-40px] text-lg right-0"
				forceAnimate={true}
				animateOnce={true}>
				Handwritten and drawn with love and passion
			</MotionElement>
			<MaskElement
				...
				<a href="https://github.com/LewisJ-git" target="_blank">
					<Image src={github_logo} alt="github logo" width={60} height={60} />
				</a>
			</MaskElement>
		</MaskElement>
	);
